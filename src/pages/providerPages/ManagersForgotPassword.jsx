...
import ManagerForgotOTPModal from '../../components/providerComponents/ManagerForgotOTPModal';
import axios from '../../config/axios'

const ManagersForgotPassword = () => {
    ...
    const submitHandler = async () => {
        ...
        try {
            const response = await axios.post("/provider/forgotPassword", {
                mobile: phone,
            });
            setOtpmodal(true)
        } catch (error) {
    ...
    return (
        <div className='w-full h-[1007px] grid lg:grid-cols-3 md:grid-cols-5 bg-white'>
            <div className='md:col-span-2 lg:col-span-1 flex flex-col items-center justify-center'>
                ...
                <Link to={'/providerlogin'} className='mt-3 underline font-semibold text-gray-600'>Cancel?</Link>
            ...
            <ManagerForgotOTPModal onClose={addServiceClose} visible={Optmodal} phone={phone} />

        </div>
    )
}

export default ManagersForgotPassword